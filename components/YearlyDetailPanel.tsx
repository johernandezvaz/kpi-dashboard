...
const thClass = "text-left py-[0.45rem] px-2 font-bold text-[0.68rem] uppercase tracking-[0.08em] text-app-muted whitespace-nowrap border-r border-r-brand-navy/15";

export default function YearlyDetailPanel({ apiUrl, extraColumnHeader }: YearlyDetailPanelProps) {
  ...
  useEffect(() => {
    ...
    fetch(apiUrl)
      ...
      .catch(() => setError("Could not load annual detail data."))
      ...
      <table className="w-full border-collapse text-[0.78rem]" aria-label="Annual metric detail">
        <thead>
          <tr className="bg-app-surface-2 border-b-2 border-b-brand-blue">
            ...
            <th className={`${thClass} text-center bg-brand-navy/5`}>Annual avg</th>
            <th className="text-center py-[0.45rem] px-2 font-bold text-[0.68rem] uppercase tracking-[0.08em] text-app-muted whitespace-nowrap">
              Status
            </th>
            {MONTH_ABBR.map((m) => (
              <th key={m} className="text-center py-[0.45rem] px-1.5 font-bold text-[0.62rem] uppercase tracking-[0.06em] text-app-muted whitespace-nowrap border-l border-l-brand-navy/10">
                {m}
              </th>
            ))}
          </tr>
            ...
            return (
              ...
              >
                ...
                <td className="py-[0.4rem] px-2 text-center font-semibold text-app-text whitespace-nowrap border-r border-r-brand-navy/10 bg-brand-navy/5">
                  {m.yearly.aggregatedValue !== null
                    ? String(Math.round(m.yearly.aggregatedValue * 1000) / 1000)
                    : "—"}
                </td>
                <td className="py-[0.4rem] px-2 text-center border-r border-r-brand-navy/10">
                  <span
                    className={`inline-flex items-center justify-center px-2 py-[0.1rem] rounded-full text-[0.65rem] font-bold text-scorecard-cell-text ${colorBg[m.yearly.color] ?? "bg-scorecard-neutral"}`}
                  >
                    {m.yearly.color === "green" ? "✓" : m.yearly.color === "yellow" ? "~" : m.yearly.color === "red" ? "✗" : "—"}
                  </span>
                </td>
                {m.months.map((mc) => (
                  <td
                    ...
                    title={mc.value !== null ? `${MONTH_ABBR[mc.month - 1]}: ${mc.value}` : undefined}
                  >
