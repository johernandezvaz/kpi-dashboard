...
export default async function RangeTotalDetailPage({
  searchParams,
}: {
  searchParams: Promise<{ plant?: string; startYear?: string; startMonth?: string; endYear?: string; endMonth?: string; dimension?: string; code?: string }>;
}) {
  const { plant, startYear, startMonth, endYear, endMonth, dimension, code } = await searchParams;

  if (!plant || !startYear || !startMonth || !endYear || !endMonth || !dimension || !code) {
    notFound();
  }

  if (dimension !== "process" && dimension !== "area") notFound();
  ...
  const params = new URLSearchParams({ plant, startYear, startMonth, endYear, endMonth, dimension, code });
  const apiUrl = `/api/scorecard/range-total?${params.toString()}`;
  ...
  const breadcrumb = `${isProcess ? "Process" : "Area"} Total · ${dimLabel} · ${plantLabel} — ${startLabel} → ${endLabel}`;
    ...
    <div className="flex flex-col min-h-screen pb-8">
      <header className="flex items-center gap-4 px-6 py-[0.875rem] bg-brand-navy border-b-2 border-b-brand-blue sticky top-0 z-10">
        ...
        <div className="flex flex-col gap-0.5 min-w-0">
          <span className="text-[0.7rem] font-semibold uppercase tracking-[0.1em] text-white/50">Range Total Detail</span>
      ...
      <main className="flex-1 flex flex-col max-w-[96rem] w-full mx-auto px-6 pt-6">
        <div className="rounded-xl border border-brand-navy/20 bg-app-surface shadow-sm overflow-hidden flex flex-col">
          <RangeDetailPanel apiUrl={apiUrl} extraColumnHeader={extraColumnHeader} />
        </div>
      </main>
    </div>
  );
