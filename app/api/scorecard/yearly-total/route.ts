...
export async function GET(request: NextRequest) {
  ...
  if (!plantCode || !year || !dimension || !code) {
    return NextResponse.json({ error: "Missing params: plant, year, dimension, code" }, { status: 400 });
  }
  ...
  try {
        ...
        ? query<YearlyMetricRow>(
            `SELECT
             ...
             WHERE vy.plant_id = $1
               AND vy.year     = $2
               AND pr.code     = $3
             ORDER BY ar.sort_order, vy.name`,
            [plantId, yearNum, codeDecoded]
          )
        : query<YearlyMetricRow>(
            `SELECT
               vy.metric_id::text,
             ...
             WHERE vy.plant_id = $1
               AND vy.year     = $2
               AND ar.code     = $3
             ORDER BY pr.sort_order, vy.name`,
            [plantId, yearNum, codeDecoded]
          ),
      isProcess
        ? query<MonthlyRow>(
    ...
    console.error("[/api/scorecard/yearly-total] error:", err);
